    ...
    .pipe(through2.obj(function(demoFolder, enc, next) {
      ...
      function toDemoObject(file) {
        return {
          ...
          outputPath: 'src/components/' + name + '/' + path.basename(demoFolder.path) + '/' + path.basename(file.path),
        };
      }
    }));
};
...
exports.buildOtUiDefinition = function() {
  var buffer = [];
  var modulesSeen = [];
  return through2.obj(function(file, enc, next) {
    ...
    var EXPLICIT_DEPS = ['ngMaterial'];
    var angularFileContents = "angular.module('ot', " + JSON.stringify(EXPLICIT_DEPS.concat(modulesSeen)) + ');';
    var angularFile = new gutil.File({
      base: process.cwd(),
      path: process.cwd() + '/ot.js',
      contents: new Buffer(angularFileContents)
    });
    angularFile.isGen = true;
